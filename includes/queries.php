            ...
            switch (esc_attr($_REQUEST["page"])) {
                case "lab_limit_attempts_booster":
                    ...
                    $last_limit = $wpdb->get_results
							...
							WHERE meta_key = %s ORDER BY meta_id DESC LIMIT 10", "recent_login_data"
                            )
                    );
                    ...
                    if (esc_attr($visitor_logs_data["visitor_logs_monitoring"]) == "enable") {
                        ...
                        $live_traffic_module = $wpdb->get_results
                                ...
                                $wpdb->prepare
                                        (
                                        "SELECT * FROM " . limit_attempts_booster_meta() . "
									WHERE meta_key = %s ORDER BY meta_id DESC LIMIT 10", "visitor_log_data"
                                )
                        );
                ...
                case "lab_last_blocked_ip_addresses":
                    ...
                    $manage_ip = $wpdb->get_results
                            ...
                            $wpdb->prepare
                                    (
                                    "SELECT * FROM " . limit_attempts_booster_meta() . "
							WHERE meta_key = %s ORDER BY meta_id DESC LIMIT 10", "block_ip_address"
                            )
                    );
                ...
                case "lab_last_blocked_ip_ranges":
                    ...
                    $manage_range = $wpdb->get_results
                            ...
                            $wpdb->prepare
                                    (
                                    "SELECT * FROM " . limit_attempts_booster_meta() . "
							WHERE meta_key = %s ORDER BY meta_id DESC LIMIT 10", "block_ip_range"
                            )
                    );
                ...
                case "lab_live_traffic" :
                    $visitor_logs_data = get_limit_attempts_booster_meta_data("other_settings");
                    if (esc_attr($visitor_logs_data["live_traffic_monitoring"]) == "enable") {
